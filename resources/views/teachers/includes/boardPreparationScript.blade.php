...
<script type="text/javascript">
    ...
    let swapRestrictText = "{{ __('teachers.boards.swapRestrictText') }}";
    let formRestrictText = "{{ __('teachers.boards.formRestrictText') }}";
    let diffRestrictText = "{{ __('teachers.boards.diffRestrictText') }}";
    ...
    $(document).on('click', '#reviewTab', function(e){
        ...
        if($('input[name=difficultyLevel]').val() != 3) {
            msgToast('info', diffRestrictText);
        } else {
            if(errorCounter == 0){
    ...
    $(document).on("click", ".assign-confirm", function() {
        ...
        let subjectName = $("select[name=select_subject] option:selected").text();
        let examinationData = createExaminationBody();

        $.ajax({
            type: "POST",
            url: "{{ route('board.assign.template.data') }}",
            data: { templateId: templateid, postdata: JSON.stringify(examinationData), pdf : true, subjectName: subjectName },
            dataType: "json",
            headers: {
                "X-CSRF-TOKEN": "{{ csrf_token() }}"
            },
    ...
    function validateQuestionLayoutInp() {
        ...
        if(!$('.test-format-slider-link').hasClass('current')) {
            ...
            errorCounter += true;
        } else if ($(".item").find(".active.current").parent(".item").data("id") != "{{ config('constants.selectedTemplateId') }}"){
            errorDiv += `<div class='toast-message'>Select ( {{ $session['inputTitle'] }} ) template</div>`;
            errorCounter += true;
        } else {
            errorCounter += false;
        }
    ...
    $(document).on('change','.get_curl_data', function(){
        if($(this).attr('name')){
            ...
            let subjectText = $('select[name=select_subject] option:selected').text();
            let route_name = $(this).data('get');
            let value = $(this).val();

            if(subjectName !==""){
                var examTitle = `{{ __('teachers.boards.mockExamTitle') }} - ${subjectText}`;
                $('input[name=input_title]').val(examTitle);
            }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if($(`select[name=${route_name}]`).hasClass('selectOptGroup')){
                            let htmlView = `<input type="text" class="form-control input-regular" value="Term - 1 Syllabus." readonly="readonly">`;
                            $(`.div-select_topic`).html(htmlView);
                        }else{
    ...
    function getDropDownData(){
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('pages.boardPreparation.selectPanel') }}",
            dataType: "html",
            success: function (response) {
    ...
    function getTemplateData(){
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('pages.boardPreparation.myTemplates') }}",
            dataType: "html",
            success: function (response) {
    ...
    $(document).on('click', '.swap-question', function(e) {
        msgToast('info', swapRestrictText);
        return false;
    })
    ...
    function reviewCarousel(item = 0){
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('board.get.questions.modal') }}",
            data:{},
            dataType: "json",
            success: function (response) {
                if(response.html) {
                    $('#owl-carousel-review').html(response.html);
    ...
    function getReviewQuestionPage() {
        ...
        let selectSubjectText  = $('select[name=select_subject] option:selected').text();
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                dataRequest.subjectName = selectSubjectText;

                $.ajax({
                    type: "POST",
                    url: "{{ route('pages.boardPreparation.paper') }}",
                    data: dataRequest,
                    dataType: "html",
                    timeout: 45000,
                    headers: {
                    ...
                    success: function (resp) {
                        ...
                        if($('.insufficient-question').length > 0) {
                            ...
                            $('#reviewPreviousBtn').click();
                            msgToast('info', `${insufficientResp}`);
                        }
                        $('[data-toggle="tooltip"]').tooltip();
                    },
                    error: function (error) {
                        $('#loader-wrapper').hide();
                        msgToast('info', `${noDataAvailable}`);
                        window.location.href = '{{ route("boardPreparation") }}'; //using a named route
                    }
                });
            },
            error: function (err) {
    ...
    function getReviewQuestionPage1() {
        ...
        if(selectedTemplateStatus.templateId != null) {
            dt = {templateId: selectedTemplateStatus.templateId, pointEventNone:""};
        } else {
            dt = {isUntimed: $(".oldpaper").data("isuntimed"), restrictresult: $(".oldpaper").data('restrictresult'), pointEventNone:"pointEventNone"};
        }
        ...
        $.ajax({
            ...
            success: function(resp){ 
                ...
                $("#customCheck102").attr({'disabled':true});
                ...
                $("#test-schedule-date").datetimepicker({
                    ...
                    minDate:new Date(),
                    maxDate:new Date('2021-11-13 11:59 PM'),
                    step:30,
                ...
                $("#test-published-on").datetimepicker({
                    format: 'ddd, MMM D, hh:mm A',
                    minDate: new Date('2021-11-29 12:00 PM'),
                    maxDate: new Date('2021-11-29 12:01 PM'),
                    onSelectDate: function(date){
                        return false;
                    },
                    onSelectTime: function(time){
                ...
                let testDueDate = new Date('2021-11-25 11:55 PM');
                let minDueDate = new Date(moment(newScheduleDate).add(0, 'days'));
                let testPublishedDate = new Date('2021-11-29 11:00 PM');

                $("#test-schedule-date").data('date', newScheduleDate).val(moment(newScheduleDate).format('ddd, MMM D, hh:mm A'));
                $("#test-due-date").data('date', testDueDate).val(moment(testDueDate).format('ddd, MMM D, hh:mm A'));
                ...
                $('input[name="schedule_minutes"]').val(90).focus().attr({"readonly":true});
                $(".scheduleResult").attr({"checked":true, "disabled":true});
            },
            error: function(data) {
                if(isOnline) {
        ...
        if(!$("div").hasClass("oldpaper")) {
            $.ajax({
                type: "POST",
                url: "{{ route('board.save.general.data') }}"+`?inputTitle=${inputTitle}&selectBoard=${selectBoard}&selectSubject=${selectSubject}&selectGrade=${selectGrade}&selectTopic=${selectTopic}&selectSubTopic=${selectSubTopic}&selectedTemplateId=${selectedTemplateId}&selectedTemplateType=${selectedTemplateType}&difficultyLevel=${difficultyLevel}&questionBank=${questionBank}&previousYear=${previousYear}&isModified=${isModified}&isSkip=${isSkip}`,
                data: templateData,
                dataType: "json",
                headers: {
                    "X-CSRF-TOKEN": "{{ csrf_token() }}"
