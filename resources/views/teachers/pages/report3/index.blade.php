        ...
        <div class="col-md-3 text-center">
            <div class="well">
                <div class="student-icon-card">
                    <img src="{!! handleProfilePic($studentData['profileUrl']) !!}">
                </div>
                <h6 class="mt-2 mb-0 f-w600"> {{ $studentData['studentName'] }} </h6>
                <h6 class="mt-2 mb-0">{{ __('teachers.report.board') }}: {{ $classReportBySubject['board']['boardName'] }} | {{ __('teachers.assessment.grade') }}: {{ $classReportBySubject['grade']['gradeName'] }}</h6>
                <h6 class="mt-2 mb-0">{{ __('teachers.assessment.subject') }}: {{ $classReportBySubject['subject']['subjectName'] }}</h6>
            </div>
        </div>
        <div class="col-md-3 attempt-count text-center">
            <div class="well pt-4 pb-4">
            <br>
                <h1 class="mb-1">{{ $studentReportBySubject['attemptedAssessment'] . '/' . $studentReportBySubject['totalAssessment'] }}</h1><br>
                                ...
                                <div class="col-md-4">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="sub-name">
                                            ...
                                            @if(isset($assessment['isAttempted']) && $assessment['isAttempted'] == true)
                                                <p title="{{ sprintf(__('teachers.report.submittedOn'), utcToDate($assessment['submissionDate'])) }} | {{ sprintf(__('teachers.report.resultPublishDate'), utcToDate($assessment['resultDate'])) }}">{{ sprintf(__('teachers.report.submittedOn'), utcToDate($assessment['submissionDate'])) }} | {{ sprintf(__('teachers.report.resultPublishDate'), utcToDate($assessment['resultDate'])) }}</p>
                                            @endif
                                        </div>
                                    ...
                                    @if(isset($assessment['isAttempted']) && $assessment['isAttempted'] == true)
                                        <div class="card-action-box">
                                            <div class="rank-badge">
                                                <div class="">{!! config('constants.icons.rank-icon') !!}</div>
                                                <h2>{{ $assessment['rank'] }}</h2>
                                            </div>
                                            <div class="progressbar ml-4">
                                                <div class="student-point circle" data-percent="{{ ($assessment['totalMark'] > 0) ?  number_format(($assessment['optainedMark'] * 100) / $assessment['totalMark']) : '0' }}">
                                                    <strong></strong>
                                                    <span></span>
                                                </div>
                                            </div>
                                            ...
                                            <div class="row-point-badge ml-2">
                                                {{ __('teachers.report.points') }}: {{ $assessment['optainedMark'] }} / {{ $assessment['totalMark'] }}
                                            </div>
                                            <a href="{{ route('report', base64_encode( $ClassRoomId.'_'. $assessment['assessmentId'] .'_'. $studentData['studentId'])) }}" class="btn btn-primary auto-width ml-4"><i class="fa fa-eye" aria-hidden="true"></i>  {{ __('teachers.report.view') }}</a>
                                        </div>
                                    @else
                                        <div class="card-action-box">
                                            <p class="text-danger"><strong>{{ __('teachers.grading.missed') }}</strong></p>
                                        </div>
                                    @endif
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function topicGraph(chunkId) {
        ...
        $.each(topicData, function (ind, dataVal) { 
            ...
            averageData[ind] = dataVal['comparitiveAverage'].toFixed(2);
            comparitiveAverageData[ind] = dataVal['average'].toFixed(2);
        });
    ...
    function assessmentGraph2(indexId) {
        ...
        $.each(assessmentData, function (ind, dataVal) { 
            var valueData = (dataVal['classAverage'] * 100 ) / dataVal['totalMark'];
            labels[ind] = dataVal['assessmentName'];
            averageData[ind] =  ((dataVal['optainedMark'] * 100) / dataVal['totalMark']).toFixed(2);
            comparitiveAverageData[ind] = `${valueData.toFixed(2)}`;
        });
            ...
            options: {
                ...
                scales: {
                    ...
                    x: {
                        ticks: {
                            callback: function(val, index) {
                                return labelsChunks[indexId][index].length > 20 ? (labelsChunks[indexId][index]).split(' ') : (labelsChunks[indexId][index]);
                            }
                        }
                    }
                }
