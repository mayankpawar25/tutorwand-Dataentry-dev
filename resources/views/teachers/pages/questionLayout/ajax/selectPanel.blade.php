            ...
            <div class="col-sm-4">
                <div class="row">
                    ...
                    @php
                        ...
                        if (isset($session['selectSubject']) && !empty($session['selectSubject'])) {
                            $disabled = "";
                        }
                    @endphp
                    <select class="form-control selectpicker onchangeValue get_curl_data" name="select_subject" data-get="select_topic" {{ $disabled }}>
                        ...
                        @if (isset($questionLayout['rightPanelData']['Subject']) && !empty($questionLayout['rightPanelData']['Subject']) && isset($session['selectSubject']))
    ...
    <div class="col-sm-5">
        ...
        <div class="div-select_topic" >
            @php
                ...
                if (isset($session['selectTopic'])){
                    $selectedTopic = explode(',', $session['selectTopic']);
                    $selectedSubTopic = explode(',', $session['selectSubTopic']);
                    $disabled = "";
                }
            @endphp
            <select class="selectOptGroup d-none onchangeValue get_curl_data" data-label="Select Topic" multiple name="select_topic" {{ $disabled }}>
                @if (isset($questionLayout['rightPanelData']['Topic']) && !empty($questionLayout['rightPanelData']['Topic']))
                    @php
                    $selectedSubTopic = ''; 
                    if(isset($session['selectSubTopic'])){
                        $selectedSubTopic = explode(',', $session['selectSubTopic']);
                    }
                    @endphp
                    @foreach ($questionLayout['rightPanelData']['Topic'] as $key => $topic)
                        @if (isset($topic['topics']) && !empty($topic['topics']))
                            <optgroup data-type="topic" data-value="{{ $topic['id'] }}" label="{{ $topic['name'] }}">
                                @foreach ($topic['topics'] as $value)
                                    @php
                                        if(is_array($selectedSubTopic) && count($selectedSubTopic)) {
                                            if(in_array($value['topicId'], $selectedSubTopic)) {
                                                $isSelected = 'selected';
                                            } else {
                                                $isSelected = '';
                                            }
                                    ...
                                    @endphp
                                    <option data-type="subtopic" data-value="{{ $value['topicId'] }}" value="{{ $value['topicId'] }}" {{ $isSelected }}>{{ $value['topicName'] }}</option>
                                @endforeach
                            </optgroup>
                        @else
                            <option data-type="topic" data-value="{{ $topic['id'] }}" value="{{ $topic['id'] }}" {{ in_array($topic['id'], $selectedTopic) ? 'selected' : '' }}>{{ $topic['name'] }}</option>
                        @endif
                    @endforeach
                @endif
            </select>
