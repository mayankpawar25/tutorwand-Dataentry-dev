...
class BoardPreparationController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $data['session'] = ['inputTitle' => 'Board Ki Taiyari'];
        ...
        return view('teachers.pages.boardPreparations.questionLayout.index', $data);
    }
    ...
    public function getCurlData(Request $request){
        ...
        if($multi){
            return view('teachers.pages.boardPreparations.questionLayout.ajax.selectMultiTags', ['datas' => $response, 'label' => $label, 'name' => $request->route_name, 'next_route' => $next_route, 'datajson' => $forFilter]);
        }
        return view('teachers.pages.boardPreparations.questionLayout.ajax.selectTags', ['datas' => $response, 'label' => $label, 'name' => $request->route_name, 'next_route' => $next_route , 'datajson' => $forFilter]);
    }
    ...
    public function assignTemplateData(Request $request) {
        ...
        if(isset($postDatas['header']['subject']['id']) && $postDatas['header']['subject']['id'] == 'nvm6nrub') {
            $pdfTitle = str_replace(" ","-", __('teachers.boards.mockExamTitle') . ' Hindi.pdf');
        } else {
            $pdfTitle = str_replace(" ","-", __('teachers.boards.mockExamTitle') . " " . $request['subjectName']) . '.pdf';
        }


        $pdfFile = asset('assets/assessmentsPdf/' . $pdfTitle);
        ...
        if(isset($response['paperId'])) {
            ...
            $pdfUrl = $this->uploadPDF($request, $pdfFile);
    ...
    public function getSelectPanel(Request $request) {
        ...
        if (!Cache::has(getStudentUserId() . ':select_board') || empty(Cache::get(getStudentUserId() . ':select_board'))) {
        ...
        if (!Cache::has(getStudentUserId() . ':select_grade') || empty(Cache::get(getStudentUserId() . ':select_grade'))) {
            $data['questionLayout']['rightPanelData']['Grade'] = getGrades(config('constants.selectedBoardId'));
            Cache::put(getStudentUserId() . ':select_grade', $data['questionLayout']['rightPanelData']['Grade'], $this->longTimeOut);
        } else {
            $data['questionLayout']['rightPanelData']['Grade'] = Cache::get(getStudentUserId() . ':select_grade');
        }
        if (!Cache::has(getStudentUserId() . ':select_subject') || empty(Cache::get(getStudentUserId() . ':select_subject'))) {
            $data['questionLayout']['rightPanelData']['Subject'] = getSubject(config('constants.selectedBoardId'), config('constants.selectedGradeId'));
            Cache::put(getStudentUserId() . ':select_subject', $data['questionLayout']['rightPanelData']['Subject'], $this->longTimeOut);
        } else {
            $data['questionLayout']['rightPanelData']['Subject'] = Cache::get(getStudentUserId() . ':select_subject');
        }

        if (!Cache::has(getStudentUserId() . ':select_topic') || empty(Cache::get(getStudentUserId() . ':select_topic'))) {
            $data['questionLayout']['rightPanelData']['Topic'] = getTopics(config('constants.selectedBoardId'), config('constants.selectedGradeId'), config('constants.selectedSubjectId'));
            Cache::put(getStudentUserId() . ':select_topic', $data['questionLayout']['rightPanelData']['Topic'], $this->longTimeOut);
        } else {
        ...
        $data['session'] = [
                                ...
                                'selectTopic' => '',
                                'selectSubTopic' => '',
                                'selectedTemplateId' => config('constants.selectedTemplateId'),
                                'difficultyLevel' => config('constants.selectedDifficultyLevel')
                            ];
        return view('teachers.pages.boardPreparations.questionLayout.ajax.selectPanel' , $data);
    }
    ...
    public function getMyTemplate(Request $request) {
        ...
        $data['questionLayout']['lastUsedTemplate'] = config('constants.selectedTemplateId');
    ...
    public function uploadPDF(Request $request, $pdfData) {
    ...
    public function reviewPaper(Request $request) {
        $data = [];
        $subjectName = $request['subjectName'];
        if(isset($request['data']['selectSubject']) && $request['data']['selectSubject'] == 'nvm6nrub') {
            $paperId = config('constants.bktHindi');
        } else {
            $paperId = config('constants.bkt' . str_replace(" ", "", ucwords($subjectName)));
        }
        ...
        $request->session()->put(getStudentUserId().':paperId', $paperId);
        Cache::put(getStudentUserId() . ':canvasData', $data['canvasData']);
        if(!empty($data['canvasData']['questions'])) {
            return view('teachers.pages.boardPreparations.questionLayout.ajax.index', $data);
        } else {
            return view('teachers.pages.boardPreparations.questionLayout.ajax.insufficientQuestion', ['message' => sprintf(__('teachers.boards.paperComingSoon'), ucwords($subjectName))]);
        }
    }
    ...
    public function questionModal(Request $request) {
        ...
        $html = view('teachers.pages.questionReview.ajax.questionModal', $data)->render();
