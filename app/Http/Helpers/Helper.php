...
function GetAADBearer() {
    ...
    if(config('constants.isProd')) {
        ...
        $curlUrl = config('constants.ExtAPIUrl').'/api/Security/GenerateToken';

        $body = [
            'validationkey' => 'w9z$C&F)J@McQfTjWnZr4u7x!A%D*G-K',
        ];
        ...
        Cache::put(getStudentUserId() . ':access_token', $response, '5560');
        return $response;
    }
