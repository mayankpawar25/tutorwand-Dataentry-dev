...
function shortQuestionWithMultiCorrectValidation(){
    ...
    var answerTitle = CKEDITOR.instances[`matAnswer`].getData();
    ...
    if(answerTitle.length == 0){
        ...
        $(".matAnswer-error").html(`<span>${enterAnswer}</span>`);

        error += true;

    } else {

        $(".matAnswer-error").html("");
...
function showAnswerVariations(textBoxId){
    ...
    setTimeout(() => {
        ...
        $('[id^=mta-variation-]').each(function(e, i) {
            let idSelector = $(this).attr('id');
            let count = $(this).data('count');
            let answerVariation = CKEDITOR.instances[idSelector].getData();
            if(answerVariation.length > 0) {
                $(".solution_variation_place").append(`<div class="ml-4"><label>Solution Variation ${count+1}: </label>${$.trim(answerVariation)}</div>`);
                if(answerVariation.length > 0 || $(answerVariation).find('img').length > 0){
                    MathJax.typeset();
                }
            }
        })
...
$(document).on("click",".submitButton", function(){
    ...
    if(dataId == `${shortQuestionWMCA}-form`){
        ...
        answerTitle = CKEDITOR.instances[`matAnswer`].getData();
...
$(document).on('click', '#add-answer-variation', function() {
    ...
    $('.answervariations').append(`<div class="row variation-row">
        <div class="col-md-12">
            <div class="form-group">
                <div class="form-group">
                    <label>Solution Variation</label>
                    <textarea data-count="${count}" class="form-control mta-variations" name="variation_short_answer[]" placeholder="Solution Variation" id="mta-variation-${count}"></textarea>
                    <button class="btn btn-danger btn-sm text-right pull-right mt-2 remove-variations">Remove</button>
                </div>
